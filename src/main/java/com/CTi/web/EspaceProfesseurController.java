...
public class EspaceProfesseurController{
   ...
   @RequestMapping(value="/docs/prof/views")
	 public String docBookmark(Model model,Principal principal,@PathParam(value="id") String id ) {
	   
	   if(id.equals("") || id.equals("0")) return "redirect:/dashboard/prof";
	   
	   Professeur p=professeurRep.getOne(Long.parseLong(principal.getName()));
	   Document doc = documentRep.getOne(Long.parseLong(id));
	   
	   if(doc==null) return "redirect:/dashboard/prof";
	   if(doc.getProprietaire().getId_user()!=p.getId_user()) return "redirect:/dashboard/prof";
	   
	   List<Utilisateur> users=doc.getEnregistrerPar();
	   
	   model.addAttribute("list",users);
	   model.addAttribute("e",doc);
	   
	   return "EspaceProfesseur/docBookmark";
